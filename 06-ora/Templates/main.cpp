...
Matrix<int,1> createMatrixWOnes(int nrows, int ncols) {
	std::cout << "creating matrix w/ ones" << std::endl;
	Matrix<int,1> m = Matrix<int,1>(nrows, ncols, 1);
	std::cout << "returning new matrix" << std::endl;
	return m;
}

Matrix<int,1> createMatrixWTwos(int nrows, int ncols) {
	std::cout << "creating matrix w/ twos" << std::endl;
	Matrix<int,1> m = Matrix<int,1>(nrows, ncols, 2);
	std::cout << "returning new matrix" << std::endl;
	return m;
}

void worker() {
	Matrix<int,1> m1(3, 4, 0);
	m1[2][3] = 10;
	std::cout << "m1 created" << std::endl;

	//Matrix m2 = m1;
	Matrix<int,1> m2(m1); // ugyanazt jelenti
	m2[2][3] = 9; // ez mindket matrixot irja!
	std::cout << "m2 created" << std::endl;

	Matrix<int,1> m3(3, 4, 0);
	...
	Matrix<int,1> m4(std::move(m3)); // azt allitjuk, hogy m3-ra
	...
	Matrix<int,1> m5(createMatrixWOnes(3, 4));
	...
	Matrix<int,1> m6(3,3,5);
	m6.print();
	m6.timesTwo();
	m6.print();

	Matrix<char,2> m7(3, 3, 'a');
	m7.print();
	m7.timesTwo();
	m7.print();
	timesThree(m7);
